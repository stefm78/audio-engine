...
def main():
    ...
    base,report=load_baseline(Path(a.baseline_audio),Path(a.baseline_report)); by={int(x["i"]):x for x in report["segments"]}; anchor=derive_anchor(base,report,out)
    ...
    for i in sorted(TARGETS):
        ...
        s2t=cosine(source_x,target_x); o2t=cosine(out_x,target_x); o2s=cosine(out_x,source_x)
        source_ratio=g["duration_seconds"]/rec[i]["technical_gate"]["duration_seconds"]
        baseline_duration=float(int(by[i]["audio_ms"])/1000.0)
        staging_ratio=g["duration_seconds"]/baseline_duration if baseline_duration else math.inf
        direction=o2t>s2t
        if not (.75<=source_ratio<=1.25 and .65<=staging_ratio<=1.50 and direction):
            raise SystemExit(f"SLOT_CAPABILITY_REJECT:{i}:{source_ratio}:{staging_ratio}:{s2t}:{o2t}")
        ...
        converted[i]=seg
        results.append({"i":i,"text":TARGETS[i],"source_sha256":rec[i]["sha256"],"output_sha256":sha256(op),"technical_gate":g,"duration_ratio_vs_source":source_ratio,"duration_ratio_vs_h1b_slot":staging_ratio,"cosine_source_to_target":s2t,"cosine_output_to_target":o2t,"cosine_output_to_source":o2s,"direction_pass":direction})
