...
class FreeVCIdentityLockTests(unittest.TestCase):
    def test_contract_is_lab_only_zero_new_tts_and_four_conversions(self):
        spec = experiment_spec()
        self.assertEqual(spec["engine"]["source_revision"], FREEVC_REVISION)
        ...
        self.assertFalse(spec["licenses"]["production_legal_qualification"])

    def test_smoke_gate_has_no_absolute_threshold(self):
        self.assertTrue(smoke_gate(0.31, 0.32)["eligible"])
        self.assertFalse(smoke_gate(0.72, 0.71)["eligible"])
        self.assertFalse(smoke_gate(0.5, 0.5)["eligible"])

    def test_topology_gate_passes_only_with_target_rank_and_emotion_invariance(self):
        refs = {
        ...
        self.assertNotIn("threshold", good)

        refs_bad_rank = {key: dict(value) for key, value in refs.items()}
        refs_bad_rank["panic:lucie"] = {"claire": 0.81, "lucie": 0.79}
        self.assertFalse(topology_gate(refs_bad_rank, pairs)["eligible"])

        pairs_bad_topology = dict(pairs)
        pairs_bad_topology["cross_sadness-contained"] = 0.90
        self.assertFalse(topology_gate(refs, pairs_bad_topology)["eligible"])
