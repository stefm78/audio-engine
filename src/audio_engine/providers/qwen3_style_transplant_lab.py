...
This intentionally mixes two prompt components that the official wrapper exposes
separately:
- speaker embedding from the frozen character anchor;
- ICL speech codes + transcript from a separate expressive style donor.

The combination is an experiment, not an upstream-supported identity guarantee.
"""
...
class Qwen3StyleTransplantLabProvider:
    ...
    def build_style_prompt(self, identity_embedding, style_wav_path, style_text: str):
        ...
        # Deliberate experimental splice: keep donor speech codes / transcript,
        # replace only its speaker embedding with the frozen character embedding.
